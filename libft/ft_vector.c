...
static int _vector_realloc(t_vector **vector_addr, t_vector_header *header,
                           size_t target_capacity) {
    t_vector_struct *new_vector;

    new_vector = realloc(header, sizeof(t_vector_header) +
                                     target_capacity * header->type_size);
    if (!new_vector)
        return (1);
...
// 	return (_vector_realloc(vector_addr, header, (header->capacity ?
// header->capacity * 2 : 2)));
// }
...
/// @param initial_capacity Number of element to reserve the space for at
/// initialization
...
/// @brief Add a new element to the vector which is a copy of memory area
/// pointed by data
/// @param vector_addr
/// @param data
...
/// @param pos
/// @param data
/// @return ```1``` if ```vector_addr``` was ```NULL``` or if a realloc
/// operation failed. Vector IS NOT freed
...
/// @brief Remove the last element of the vector. Safe to use with an empty
/// vector.
/// @param vector_addr
/// @return ```1``` if ```vector_addr``` was ```NULL``` or if a shrinking
/// operation failed. Vector IS NOT freed.
...
int ft_vector_erase_range(t_vector **vector_addr, size_t pos, size_t n) {
    ...
    if (pos + 1 != header->len)
        ft_memmove(*vector_addr + pos * header->type_size,
                   *vector_addr + (pos + n) * header->type_size,
                   header->len - (pos + 1));
    header->len -= n;
...
int ft_vector_insert_range(t_vector **vector_addr, size_t pos, const void *data,
    ...
    if (pos < header->len)
        ft_memmove(*vector_addr + (pos + n) * header->type_size,
                   *vector_addr + pos * header->type_size,
                   (header->len - pos) * header->type_size);
    ft_memcpy(*vector_addr + pos * header->type_size, data,
              header->type_size * n);
    header->len += n;
    return (0);
}
...
/// @brief Set a vector capacity. Nothing is done if the actual size or capacity
/// of the vector is bigger than desired.
/// @param vector_addr
/// @param nbr_el
/// @return ```1``` if ```vector_addr``` was ```NULL``` or if a realloc
/// operation failed. Vector IS NOT freed
...
int ft_vector_resize(t_vector **vector_addr, size_t size) {
    ...
    header->len = size;
    if (_has_over_capacity(header)) {
        if (_vector_realloc(vector_addr, header, header->len) == 1)
            return (1);
    }
    return (0);
}
...
void ft_dump_vector(t_vector *vector, bool print_capacity) {
    ...
    ft_sdprintf(1, "size = %d/%d | type_size = %lu\n", header->len,
                header->capacity, header->type_size);
    ft_hexdump_color_zone(vector,
                          (print_capacity ? header->capacity : header->len) *
                              header->type_size,
